...
def all_gather_list(py_list):
    """
    使用 all_gather_object 收集各 rank 的 python 列表，并拼接为一个总列表。
    需要 torch.distributed 已初始化。
    """
    if not dist.is_available() or not dist.is_initialized():
        return list(py_list)

    world_size = dist.get_world_size()
    gather_container = [None for _ in range(world_size)]
    dist.all_gather_object(gather_container, list(py_list))
    merged = []
    for part in gather_container:
        merged.extend(part)
    return merged
...
def load_lip(path, target_frames=None):
    """
    加载视频并均匀采样指定数量的帧。如果总帧数小于 target_frames，则重复最后一帧补齐。

    参数:
        path (str): 视频文件路径
        target_frames (int): 目标帧数（None表示不采样，返回所有帧）

    返回:
        np.ndarray: 形状为 (target_frames, H, W) 的灰度帧数组
    """
    for attempt in range(3):
        ...
        try:
            ...
            if not cap.isOpened():
                raise cv2.error(f"无法打开视频文件: {path}")
            ...
            if target_frames is not None:
                if frames.shape[0] >= target_frames:
                    # 均匀采样
                    indices = np.linspace(0, frames.shape[0] - 1, target_frames, dtype=int)
                    frames = frames[indices]
                else:
                    # 复制最后一帧补齐
                    pad_len = target_frames - frames.shape[0]
                    last_frame = np.repeat(frames[-1][np.newaxis, ...], pad_len, axis=0)
                    frames = np.concatenate([frames, last_frame], axis=0)

            return frames

        except Exception as e:
            print(f"第 {attempt + 1}/3 次尝试失败: {e}")
            if attempt == 2:
                raise ValueError(f"无法加载视频: {path}") from e
        finally:
            if cap is not None:
                cap.release()
...
def load_video(path, target_frames=None, color_mode="rgb"):
    """
    加载视频并返回指定格式的帧数组
    ...
    参数:
        path (str): 视频文件路径
        target_frames (int): 目标帧数（None表示不采样，返回所有帧）
        color_mode (str): "rgb"（彩色）或 "grayscale"（灰度）

    返回:
        np.ndarray:
            - 如果 color_mode="rgb": (T, 3, H, W)
            - 如果 color_mode="grayscale": (T, 1, H, W)

    异常:
        ValueError: 视频加载失败或帧数不足
    """
    for attempt in range(3):
        cap = None
        try:
            ...
            # 根据 color_mode 确定转换方式和通道数
            if color_mode == "rgb":
                convert = lambda x: cv2.cvtColor(x, cv2.COLOR_BGR2RGB)
                channels = 3
            elif color_mode == "grayscale":
                convert = lambda x: cv2.cvtColor(x, cv2.COLOR_BGR2GRAY)
                channels = 1
            else:
                raise ValueError("color_mode 必须是 'rgb' 或 'grayscale'")

            # 如果未指定 target_frames 或视频帧数不足，则读取所有帧
            if target_frames is None or target_frames >= total_frames:
                ...
                # 预分配数组 (T, H, W, C) → 之后会转置为 (T, C, H, W)
                frames = np.zeros((target_frames, height, width, channels), dtype=np.uint8)

                # 均匀采样
                sampled_indices = np.linspace(0, total_frames - 1, target_frames, dtype=int)
                for i, pos in enumerate(sampled_indices):
                    cap.set(cv2.CAP_PROP_POS_FRAMES, pos)
                    ret, frame = cap.read()
                    if ret:
                        frames[i] = convert(frame)
                    else:
                        raise cv2.error(f"读取第 {pos} 帧失败")
